...
| TR-1 — Successor use cases | T-01, T-02, T-03 | Add separate actor/outcome/exception/acceptance use cases for DateTime/DST; resource capacity and explicit leveling; cost/time observations; collaboration conflict resolution; authorization/approval/audit; hosted synchronization. | Updated catalog and traceability entries with no empty successor use-case list. |
| TR-2 — Milestone ownership | T-04, T-05, T-06, T-07, T-10, T-11 | Assign AI integration and Actual ingestion; make M0.5→M1 gating explicit; link FD-4/FD-5 to M8/M9; add post-M9 successor milestones in dependency order; correct the stale post-M9 boundary. | Roadmap rows with outcomes, deliverables, reuse boundaries, and exit evidence for every mapped use case. |
