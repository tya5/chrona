...
| M0.5 — Self-hosted delivery grammar | A team can represent and review its Chrona delivery work as a Chrona Project without changing scheduling or Actual semantics. | Standard `implementation-delivery` profile, bounded typed fields, self-hosted delivery-plan fixture, resolved-manifest validation, and typed-field Command support. Store-bound package resolution completes with M1. | Core scheduling, Actual separation, Revision Store identities, standard Command families, diagnostics. | Profile fixtures prove state isolation, immutable evidence, deterministic self-hosted plan evaluation, and—after M1—pinned package resolution; cross-document self-hosting review passes. |
