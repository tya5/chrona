    ...
    with tempfile.TemporaryDirectory(dir=root) as temporary:
        ...
        for kind, filename in names.items():
            if kind not in sources:
                continue
            # Guided ingress derives its identities from canonical normalized YAML.
            # Preserve that identity here: category-color allocation is intentionally
            # identity-stable, so a presentation bundle cannot use presentation-only
            # key ordering as an accidental input.
        ...
        explicit_draft = resolve_draft_render(
            project_path=staged / "project.yaml", view_path=staged / "view.yaml",
            theme_path=staged / "theme.yaml", scheme_path=staged / "scheme.yaml",
            layout_path=staged / "layout.yaml",
            actual_path=(staged / "actual.yaml") if "actual-set" in written else None,
        )
...
def _render_bytes(draft: Any) -> bytes:
    ...
    rendered = render_review(RenderRequest(
        closure, draft.asset_root, ReferenceScheduler(), renderer_for(
