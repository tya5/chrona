...
def measure_sources(inputs: Mapping[str, SourceInput], theme: Mapping[str, Any], *, font_metrics: Any) -> MeasuredSources:
    ...
    body_line = metric["text.body.size"] * metric["text.body.lineHeight"]
    average_advance = Decimal(str(font_metrics.width("M", float(metric["text.body.size"]))))
    metric["text.measuredAverageAdvance"] = average_advance
    result: dict[str, Measurement] = {}
    for source, value in sorted(inputs.items()):
        measured_width = max((Decimal(str(font_metrics.width(line, float(metric["text.body.size"])))) for line in value.lines), default=average_advance)
        text_inline = max(average_advance, measured_width)
        text_block = max(body_line, Decimal(max(1, len(value.lines))) * body_line)
        if source == "table":
        ...
        result[source] = Measurement(
            ...
            min(preferred_block, body_line), preferred_block, preferred_block * 2,
            Decimal(str(font_metrics.baseline(0, float(metric["text.body.size"]), float(metric["text.body.lineHeight"])))),
            Decimal(str(font_metrics.baseline(0, float(metric["text.body.size"]), float(metric["text.body.lineHeight"])))),
        )
