...
def _load_reference(reference: dict[str, Any], reader: SnapshotReader, expected_kind: str,
    ...
    if expected_kind == "project":
        ...
        if not isinstance(value, dict) or value.get("version") != "chrona/layout-profile/v0.6":
            raise ClosureError("E_CLOSURE_KIND", detail=f"reference id={reference.get('id')!r}; expected chrona/layout-profile/v0.6 object; found {value!r}")
        actual_id = value.get("id")
    else:
