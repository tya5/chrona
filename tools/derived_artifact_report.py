...
def report_stale_artifact(*, output: Path, generated: str, code: str,
    ...
    diff, truncated = _bounded(unified_diff(committed.splitlines(keepends=True), generated.splitlines(keepends=True),
                                            fromfile=f"committed/{report_path}", tofile=f"generated/{report_path}"))
    print(f"{code}: {report_path}")
    if diff:
    ...
    if os.environ.get("GITHUB_ACTIONS") == "true":
        print(f"::error file={report_path}::{code}; refresh with " + json.dumps(list(refresh_argv), ensure_ascii=False))
