...
PRESENTATION_DIRECTORIES = ("views", "themes", "layouts", "schemes", "profiles")

# An example presentation file that no Context or packaged preset renders must
# say why it is kept (#434).  An entry without a file, or a file without an
# entry, fails ``--check``.
UNREFERENCED_REASONS: dict[str, str] = {
    "examples/aster-ssd/themes/onboarding-variation.yaml":
        "derived-Theme (v0.12 inheritance) example used by `docs/guides/first-project.md` and `tests/integration/test_render.py`",
    "examples/controller-z/profiles/summary.yaml":
        "Summary Profile input exercised by `tests/integration/test_render.py` and the contract-resource tests, not by a committed slide",
}


def _collect_addresses(value: Any) -> Iterable[str]:
    if isinstance(value, Mapping):
        address = value.get("address")
        if isinstance(address, str):
            yield address
        for child in value.values():
            yield from _collect_addresses(child)
    elif isinstance(value, list):
        for child in value:
            yield from _collect_addresses(child)


def unreferenced_presentation_files(root: Path) -> tuple[str, ...]:
    """Return example View/Theme/Layout/Scheme/profile files no Context or preset renders."""
    referenced: set[Path] = set()
    for manifest_path in sorted((root / "examples").glob("*/manifest.yaml")):
        project_root = manifest_path.parent
        for context_path in _context_paths(project_root, _load(manifest_path)):
            referenced.update((project_root / address).resolve() for address in _collect_addresses(_load(context_path)))
    presets = root / "src/chrona/resources/presets"
    for preset_path in sorted(presets.glob("*.yaml")):
        text = preset_path.read_text(encoding="utf-8")
        for source_root, source_path in re.findall(r"sourceRoot: ([^,}\s]+), sourcePath: ([^,}\s]+)", text):
            base = root / source_root if source_root.startswith("examples") else presets.parent / source_root
            referenced.add((base / source_path).resolve())
        for path in re.findall(r"path: ([^,}\s]+)", text):
            referenced.add((root / "examples/halcyon-1" / path).resolve())
    for theme_path in sorted((root / "examples").glob("*/themes/*.yaml")):
        base = _load(theme_path).get("body", {}).get("extends")
        if isinstance(base, Mapping) and isinstance(base.get("path"), str):
            referenced.add((theme_path.parent / base["path"]).resolve())
    files = sorted(path for directory in PRESENTATION_DIRECTORIES
                   for path in (root / "examples").glob(f"*/{directory}/*.yaml"))
    return tuple(path.relative_to(root).as_posix() for path in files if path.resolve() not in referenced)


def validate_unreferenced(root: Path) -> None:
    unreferenced = set(unreferenced_presentation_files(root))
    missing, stale = sorted(unreferenced - set(UNREFERENCED_REASONS)), sorted(set(UNREFERENCED_REASONS) - unreferenced)
    if missing or stale:
        raise CorpusCoverageError("E_CORPUS_UNREFERENCED_EXAMPLE:" + ",".join(missing + [f"stale:{item}" for item in stale]))
...
def render(root: Path) -> str:
    ...
    lines.extend(["", "## Unreferenced example presentation files", "",
    ...
    unreferenced = unreferenced_presentation_files(root)
    lines.extend(f"- `{path}`: {UNREFERENCED_REASONS.get(path, 'NO DECLARED REASON')}" for path in unreferenced)
    if not unreferenced: lines.append("None.")
    lines.append("")
    return "\n".join(lines)
