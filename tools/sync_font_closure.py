...
def synchronize(descriptor_path: Path, context_path: Path) -> None:
    ...
    if descriptor.get("algorithm") != "declared-metrics-v2" or not isinstance(assets, list) or not isinstance(environment, dict):
        raise FontClosureSyncError("E_FONT_CLOSURE_SYNC_SCHEMA")
    current = environment.get("fontMetrics")
    if not isinstance(current, dict) or current.get("algorithm") != "declared-metrics-v2":
        raise FontClosureSyncError("E_FONT_CLOSURE_SYNC_SCHEMA")
    expected = {(item.get("family"), item.get("weight")) for item in assets if isinstance(item, dict)}
    actual = {(item.get("family"), item.get("weight")) for item in current.get("assets", ()) if isinstance(item, dict)}
    if not expected or actual != expected:
        raise FontClosureSyncError("E_FONT_CLOSURE_SYNC_TARGET")
    descriptor_text = yaml.safe_dump(descriptor, sort_keys=False, allow_unicode=True).rstrip().splitlines()
