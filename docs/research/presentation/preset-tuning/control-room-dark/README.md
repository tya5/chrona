...
HALCYON-1 renders at a requested 1600 × 900 with no CLI warnings or layout failures. Its Scene still records `W_LAYOUT_ACTUAL_INCOMPLETE:tvac` and two declared `W_LAYOUT_LABEL_SUPPRESSED` outcomes. The starter keeps its one pre-existing as-of label warning.
