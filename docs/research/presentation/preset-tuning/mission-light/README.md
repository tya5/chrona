...
HALCYON-1 renders at a requested 1600 × 900 with no CLI warnings or layout failures. Its Scene still records the truthful `W_LAYOUT_ACTUAL_INCOMPLETE:tvac` fact and two declared `W_LAYOUT_LABEL_SUPPRESSED` outcomes. The starter keeps one warning it already had (`W_LAYOUT_LABEL_OVERFLOW` on the as-of label).
