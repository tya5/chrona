...
- **closure** (`presentation/model/closure.py`) resolves and verifies every
  declared input against its schema and, when pinned, its content identity.
...
- **render** (`presentation/renderers/`) maps primitives to an output format and
  makes no layout decision.

Each stage consumes the previous stage's result and nothing else. Text is
measured once, in Layout, with the font metrics the closure pinned.
...
| A contract field | the JSON Schema, then the one module that reads it |
| A pipeline | a module under `usecases/`, and a thin handler in `app/cli.py` |
