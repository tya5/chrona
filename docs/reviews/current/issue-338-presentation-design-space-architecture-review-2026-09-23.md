...
| `density`, label behavior, and adaptive axis could be mistaken for coordinates. | View has intent-level `layoutIntent`/visibility; Layout owns measurement, fitting, placement, and routing. | Keep only intent/profile choices authored.  Coordinates, metrics, collision decisions, and routes remain derived. |
