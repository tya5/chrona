...
| R350-10 bounded raster | Controller Z `programme-mark.png`, catalog/closure tests | accepted; decoded PNG/artifact-size audit remains R6 |
...
The current public Controller Z icon slide demonstrates a vector title visual
and raster mark. R6 must still add public imported Lucide/Tabler corpus
fixtures, an encoded-selection example, and explicit decoded PNG/artifact-size
checks. These are not implied by the implementation tests above.
